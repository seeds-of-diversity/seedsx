...
$(document).ready(function(){
	...
	loc = slivPathGetName();
	if(loc){
		if(loc == "login-1") login();
		else{
			if (eval("typeof " + loc + " == 'function'")) {
				eval(loc+"()");
			} else {
				loc = loc.split("-");
...
function slivPathGetName()
{
	...
	var m = window.location.href.match( /overlord=([^&]*)/ ); 
	if( m ) {
		ret = m[1];
	} else {
		s = /\/sl2\/([^/\?]*)/;    // Eclipse syntax doesn't like the \? but it's a literal ? to terminate "/sl2/foo?a=b", not a regex specifier.
		                           // If Eclipse stops highlighting an error here, please remove this comment.
		m = window.location.pathname.match( s );
		if( m ) {
			ret = m[1];
		}
	}
...
{
	// www.seedliving.ca			/sl2/page
	// www.seeds.ca/seedliving		/seedliving/sl2/page
	// localhost/.../seedliving		/.../seedliving/sl2/page
    ...
    if( window.location.host == "localhost" ) {
    	...
    	var ra = pth.split("/");
    	while( ra.length ) {
    		if( ra[ra.length-1] == "sl2" )  break;
    		ra.splice( ra.length-1, 1 );  // remove the last array element
    	}
    	pth = "/" + ra.join("/");
		...
		$.ajax({
			...
			success: function(data){
				if(data=="0") alert("Error: All purchased items have to be in the same currency. Meaning you can not have items in your cart of different currency. We are sorry for the inconvenience and are currently looking to resolve this matter.");
					...
					$("#cartTotal").html(data);
					alert("Item has been added to your cart");
					window.location = slivPathMake("mycart")+"/";
				}
			}
		});			   
	});
...
function login(){
	$(document).bind("keydown", function(e) {
		if (e.keyCode == 13) {  
			var urlAuth = slivPathMake( "authenticate" );
			var urlUser = slivPathMake( "secureUser" );

			var formData = $("#loginForm").serialize();
			$.ajax({
				type: "POST",
				url: urlAuth, //"/sl/authenticate/",
				data: formData,
				success: function(data){
					init();
					if(data) $(".failure").html(data);
					else window.open( urlUser/*"/sl/secureUser/"*/,"_self");
				}
			});
