...
$p_step = SEEDSafeGPC_Smart( "step", array( 0, 1 ) );

/* Validate parms
 */
if( $p_step == 1 ) {
    if( !($p_cmp_id = SEEDSafeGPC_GetInt( "cmp_id" )) )     $p_step = 0;
 ...
 */
if( $p_step == 0 ) {
    echo "<H2>Upload a seed file</H2>";
    echo "<FORM enctype='multipart/form-data' method='POST'>";
    echo "<INPUT type=hidden name=step value=1>";
    echo "<INPUT type='hidden' name='MAX_FILE_SIZE' value='1000000' />";
    echo "<TABLE>";
    echo "<TR><TD valign='top'>Company</TD><TD><SELECT name='cmp_id'>";
    echo "<OPTION value='' SELECTED>--- Choose ---</OPTION>";
    $dbc = $kfdb->KFDB_CursorOpen("SELECT rl_cmp_id,name_en FROM rl_companies WHERE _disabled=0");
    while( $ra = $kfdb->KFDB_CursorFetch( $dbc ) ) {
        echo "<OPTION value='${ra['rl_cmp_id']}'>${ra['name_en']}</OPTION>";
    }
    echo "</SELECT><BR><BR></TD></TR>";

    echo "<TR><TD colspan=2><INPUT type=radio name='dropRows' value=0 checked> Add to this company's list</TD></TR>";
    echo "<TR><TD colspan=2><INPUT type=radio name='dropRows' value=1> Completely replace this company's list<BR><BR></TD></TR>";

    echo "<TR><TD>Upload this file: </TD><TD><INPUT name='csvfile' type='file' size='40'/></TD></TR>";
    echo "<TR><TD><BR><INPUT type='submit' value='Upload'/></TD></TR>";
    echo "</TABLE></FORM>";
