<?
/* Seeds of Diversity's Online Document Library

   This portal page can do several things:
       seedcat = index    : show the index of Classic Canadian Seed Catalogues
       seedcat = doc_code : show the presentation page for a Classic Catalogue where doc_code is either doclib_document.doc_code or doclib_document._key

       mag     = index    : show the magazine index
       mag     = mag_code : show a magazine issue, article, or page

       else serve PUB pages of the Online Library DocRepWebSite ( or maxver pages if ?test=1 )
*/


if( !defined("SITEROOT") )  define("SITEROOT", "../");
include_once( SITEROOT."site.php" );
include_once( STDINC."SEEDFile.php" );
...
if( !empty($_REQUEST['seedcat']) ) {
    // Seed Catalogue mode
    list($kfdb) = SiteStart();

    $page1parms['sBody'] = drawSeedcatPage( $kfdb );
    if( !empty($page1parms['sBody']) ) {
        echo Page1Str( $page1parms );
        $bPage1Done = true;
    }

} else if( !empty($_REQUEST['mag']) ) {
    // Magazine mode
    list($kfdb) = SiteStart();
    $page1parms['sBody'] = "<H2>Seeds of Diversity Magazine</H2>";
    if( !empty($page1parms['sBody']) ) {
        echo Page1Str( $page1parms );
        $bPage1Done = true;
    }
...
if( !$bPage1Done ) {
    ...
    $raDWparms = array( "lang"    => $lang,
                        "docid_home" => ($lang == "FR" ? "web/libfr_home" : "web/lib_rootfolder/home"),
                        "docid_root" => ($lang == "FR" ? "web/libfr_rootfolder" : "web/lib_rootfolder"),
...
function drawSeedcatPage( $kfdb )
{
    ...
    if( $p_seedcat == 'index' ) {
        ...
        $s = "<TABLE border='0'><TR><TD valign='center'><H2>Classic Canadian Seed Catalogues</H2></TD>"
            ."<TD>".SEEDStd_StrNBSP("",10)."</TD>"
            ."<TD valign='center'><FORM method='get' action='${_SERVER['PHP_SELF']}'>"
        ...
        if( ($kfr = getDocLibKFRC( $kfdb, "doc_type='seedcat'", array('sSortCol'=>($p_sort=='n' ? 'title' : 'pub_year')) )) ) {
            while( $kfr->CursorFetch() ) {
                $s .= $kfr->Expand( "<DIV style='margin:0 0 10px 5em'><A HREF='${_SERVER['PHP_SELF']}?seedcat=[[doc_code]]'>" )
        ...
        if( ($kfr = getDocLibKFRC( $kfdb, is_numeric($p_seedcat) ? "_key='$p_seedcat'" : "doc_code='$p_seedcat'" )) ) {
            ...
            if( $kfr->value('doc_type') == 'seedcat' ) {
                $dirreal = realpath( dirname($_SERVER['SCRIPT_FILENAME']))."/doc/sc/".$kfr->value('doc_code')."/";
                $dirurl = dirname($_SERVER['PHP_SELF'])."/doc/sc/".$kfr->value('doc_code')."/";
                ...
                $s = "<TABLE border='0'><TR><TD valign='center'><H2>Classic Canadian Seed Catalogues</H2><H3 style='margin-left:3em'>".$kfr->value('title')."</H3></TD>"
                    ."<TD>".SEEDStd_StrNBSP("",20)."</TD>"
                    ."<TD valign='center'><P><A HREF='${_SERVER['PHP_SELF']}?seedcat=index'>Back to Index</A></P></TD></TR></TABLE>"
                ...
                $raPagesN = array();
                $raPagesOther = array();
                foreach( $oFile->raTraverseItems as $ra ) {
    ...
    $s .= "<BR/><BR/><BR/><DIV style='font-size:9pt; border:1px solid #888;background-color:#eee; margin-left:3em; padding:0 1em;'>"
         ."<P>Our thanks to the Royal Botanical Gardens, and The Garden Institute of British Columbia for their generous assistance.</P>"
         ."<P>This work was carried out with the aid of a grant from the "
         ."<A HREF='http://www.idrc.ca' target='_blank'>International Development Research Centre, Ottawa, Canada</A></P></DIV>";
