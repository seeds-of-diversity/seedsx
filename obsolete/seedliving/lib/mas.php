...
function mas_gri( &$mas, &$tt, &$ctt, $options, $tablename){
	...
	if( $options == XPMYSQL_NOISSUE ) {
		...
		if($mas->mas_query($sql,0)) $retcode =  mysql_insert_id( $mas->mas_dbsock );
	}
...
//		mail("colin@anlanda.com","Matt Fucked something up","Could not connect to mas socket, check matts changes");
		return false;
	}else return true;
}
