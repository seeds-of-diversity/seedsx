 ...
 *
 * Copyright (c) 2014-2016 Seeds of Diversity Canada
 *
 */
...
class MyConsole extends Console01KFUI
{
    ...
    function __construct( KeyFrameDB $kfdb, SEEDSession $sess, $raParms )
    {
        ...
        $this->oSLDB = new SLDB_Master( $kfdb, 0 );
...
$oDS->SetValue( 'psp', $psp );
    ...
    private function drawStats( $nSLAcc, $nSLCV1, $nSLCV2, $nSLCV3 )
    {
        $s = "Seed Library accessions: $nSLAcc<br/>"
            ."Source list records: "
            .($nSLCV1 ? "PGRC, " : "")
            .($nSLCV2 ? "NPGS, " : "")
            .("$nSLCV3 compan".($nSLCV3 == 1 ? "y" : "ies"));
...
// see it in the list  .($oForm->GetKey() ? ("<tr><td colspan='2'><b>Cultivar #".$oForm->GetKey()."</b></td></tr>") : "")
    ...
    function SpeciesFormDraw( $oForm )
    {
        $sStats = "";
        if( ($kSp = $oForm->GetKey()) ) {
            $nSLAcc = $this->kfdb->Query1( "SELECT count(*) FROM seeds.sl_accession A,seeds.sl_pcv P WHERE P.fk_sl_species='$kSp' AND P._key=A.fk_sl_pcv" );
            $sSql = "SELECT count(*) FROM seeds.sl_cv_sources CV,seeds.sl_pcv P WHERE P._key=CV.fk_sl_pcv AND P.fk_sl_species='$kSp'";
            $nSLCV1 = $this->kfdb->Query1( $sSql." AND fk_sl_sources='1'" );
            $nSLCV2 = $this->kfdb->Query1( $sSql." AND fk_sl_sources='2'" );
            $nSLCV3 = $this->kfdb->Query1( $sSql." AND fk_sl_sources>='3'" );

            $sStats = "<div style='border:1px solid #aaa;display:inline-block;margin-left:10px;padding:10px'>"
                     .$this->drawStats( $nSLAcc, $nSLCV1, $nSLCV2, $nSLCV3 )
                     ."</div>";
        }
