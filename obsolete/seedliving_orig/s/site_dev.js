...
function init(){
	...
	$(".basket").click(function(){
		$.ajax({
			type: "POST",
			url: "/sl_dev/updateCart-"+$(this).attr("seedid")+"/",
		...
		$.ajax({
			...
			success: function(data){
				if(data=="0") alert("You must register and be a validated SeedLiving member in order to swap.");
						...
						if(answer){
							window.open("/sl_dev/slPurchaseSwaps/","_self");	   
						}
				} else {
					window.open("/sl_dev/slDoTTSwap-"+id+"/","_self");
				}
			}
		});
	});
   ...
   return IsNumber;
	...
	$('#slSetFeature').click(function() { 
 		if($("#featureSeedSelect").val()){
			window.open("/sl_dev/setFeatured-"+$("#featureSeedSelect").val(),"_self");
		} else {
	...
	$('.slRemoveFeature').click(function() { 
		window.open("/sl_dev/removeFeatured-"+$(this).attr("seedid"),"_self");
	});
		...
		if($(this).val()){
		  ...
		  if(cartid) {
			  $("#slCartActions")[0].selectedIndex = 0;
			   ...
			   if(cartC>0) $("#cartTotal").html("<a href=\"/sl_dev/mycart/\">"+cartC+"</a>");
			   else {
					window.open("/sl_dev/emptycart","_self");   
			   }
		  }
...
function login(){
	$(document).bind("keydown", function(e) {  
		if (e.keyCode == 13) {  
			...
			$.ajax({
				type: "POST",
				url: "/sl_dev/authenticate/",
				data: formData,
				success: function(data){
					init();
					if(data) $(".failure").html(data);
					else window.open("/sl_dev/secureUser/","_self");
				}
			});
...
function account(){
	...
	$('#registerButton').click(function() { 
		...
		$.ajax({
			type: "POST",
			url: "/sl_dev/saveAccount/",
			data: formData,
			success: function(data){
				init();
				if(data) $(".failure").html(data);
				else window.open("/sl_dev/login/","_self");
			}
		});
	});
}
