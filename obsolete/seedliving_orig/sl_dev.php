...
define( "SEEDLIVING_ROOT", "/home/154977/domains/seedliving.ca/html/" );
require_once(SEEDLIVING_ROOT."lib/lib_dev.php");
...
//if(!strcmp(ttn($gtt,"REMOTE_ADDR"),"68.144.69.157")) printf("SELECT * FROM cats WHERE instr(cat_url ,'%s')",ttn($tt,"overlord"));
if($mas->mas_row_cnt){
...
/* Load tags */
$content = file_get_contents(SEEDLIVING_ROOT."includes/tags.html"); //"/var/www/vhosts/seedliving.ca/httpdocs/includes/tags.html");
...
switch(ttn($tt,"overlord")){
		...
		mas_qb($mas,"SELECT * FROM carts,seeds WHERE seed_id = cart_seedid AND cart_userid = '%s'",ttn($gtt,"account_id"));
			while(mas_qg($mas,$temptt)){
				if(!strcmp(ttn($temptt,"cart_quantity"),ttn($temptt,"seed_quantity"))){
					...
					$dev_null = system("php -q /var/www/vhosts/seedliving.ca/httpdocs/bin/tags.php");
				} else{
		...
		for($c=0;$c<count($ids);$c++){
			...
			if(!strcmp(ttn($tt,"action"),"Delete")) {
				...
				if(!strcmp(ttn($tt,"table"),"seeds")) {
					unlink("/var/www/vhosts/seedliving.ca/httpdocs/i/seeds/".$ids[$c]."_1.jpg");
					unlink("/var/www/vhosts/seedliving.ca/httpdocs/i/seeds/thmb/".$ids[$c]."_1.jpg");
					unlink("/var/www/vhosts/seedliving.ca/httpdocs/i/seeds/".$ids[$c]."_2.jpg");
					unlink("/var/www/vhosts/seedliving.ca/httpdocs/i/seeds/".$ids[$c]."_3.jpg");
					$dev_null= system("php -q /var/www/vhosts/seedliving.ca/httpdocs/bin/tags.php");
				}
			}
		}
	...
	case "slLinks":
		if(ttn($tt,"link_name") && ttn($tt,"link_url")){
			...
			$fp = fopen("/var/www/vhosts/seedliving.ca/httpdocs/includes/links.html","a");
		...
		tkntbl_add($tt,"page",file_get_contents("/var/www/vhosts/seedliving.ca/httpdocs/includes/links.html"),1);
		tmplt_proc_ex(TEMPLROOT,tkntbl_search($tmpl,"Top"),OPENTAG,CLOSETAG,stdout,1,array(&$tt,&$gtt));
	...
	case "newSave":
		...
		if(ttn($gtt,"account_id")){
			...
			tkntbl_snprintf($tt,"new_desc",1,MAX_RESULTS,"%s",strip_tags(addslashes(stripslashes(ttn($tt,"new_desc"))),"<a><i><p><b><h1><h2><h3><strong><br>"));
			...
			while(list($key,$value) = each($_FILES[new_image][name]))
			{
				if(!empty($value)){   // this will check if any blank field is entered
					...
					$add = "/var/www/vhosts/seedliving.ca/httpdocs/i/news/".ttn($tt,"@id")."_".$c.".jpg";   // upload directory path is set
		...
		while(list($key,$value) = each($_FILES[seed_image][name]))
		{
			if(!empty($value)){   // this will check if any blank field is entered
				...
				if($c==1){
					...
					$image->save("/var/www/vhosts/seedliving.ca/httpdocs/i/seeds/thmb/".ttn($tt,"@id")."_".$c.".jpg");
				}
			}
	...
	case "saveAccount":
		...
		if(!$mas->mas_row_cnt){
			...
			tkntbl_add($tt,"email_from","seedliving_accounts@seeds.ca",1);
			tkntbl_add($tt,"email_subject","Welcome to SeedLiving",1);


			tmplt_proc_ex_tt(TEMPLROOT,tkntbl_search($tmpl,"secureIntroEmail"),OPENTAG,CLOSETAG,1,stdout,$tt,"message",array(&$tt,&$ftt,&$temptt,&$dtt));
			$headers  = "MIME-Version: 1.0\n";
			$headers .= "Content-type: text/html; charset=iso-8859-1\n";
		...
		if(ttn($gtt,"account_id")){
	    	if(ttn($tt,"@id")){
				...
				if($mas->mas_row_cnt){
					...
					//$headers .= "Bcc: colin@anlanda.com\n";
					$headers .= "From: SeedLiving <".SITEEMAIL.">\n";
					$subject = "SeedLiving - Your Trade Table request for ".ttn($temptt,"seed_title")." has been shipped.";
					mail(NULL,$subject,ttn($tt,"message"),$headers);
	...
	case "secureSwap":
		...
		if($mas->mas_row_cnt) tkntbl_add($tt,"secureSwapRow","<h2>Trade Table Requests</h2>",2);
		while(mas_qg($mas,$temptt)){
	...
	case "seedsSplash":
		$c=0;
		if(!strcmp(ttn($gtt,"fee_enabled"),"N")){
			mas_qb($mas,"SELECT * FROM seeds,accounts WHERE seed_userid = account_userid AND seed_quantity > 0 AND seed_tradetable = 'N' ORDER BY rand() LIMIT 6");
		} else {
			mas_qb($mas,"SELECT * FROM seeds,accounts WHERE seed_userid = account_userid AND seed_featured = 'Y' AND seed_tradetable = 'N' AND seed_quantity > 0 ORDER BY seed_tsmod LIMIT 6");
		}
	...
	case "tagSearch":
		$c=0;
		mas_q1($mas,$temptt,"SELECT count(*) as Total FROM tagrel, seeds, accounts WHERE seed_quantity > 0 AND account_userid = seed_userid AND seed_id = tagrel_seedid AND seed_enabled = 'Y' AND tagrel_tagid = '%s'",ttn($tt,"@id"));
		mas_q1($mas,$temptt,"SELECT count(*) as Total FROM seeds WHERE seed_quantity > 0  AND seed_enabled = 'Y' AND INSTR(seed_title,'%s')",ttn($tt,"overlord2"));

		tkntbl_add($temptt,"Total",(ttn($temptt,"Total")+ttn($temptt,"Total")),1);
		$page = ttn($tt,"page");
		$totalperpage = "18";
		if(!$page) $page = 1;
		$offset = ($page*$totalperpage)-$totalperpage;
		...
		if(ttn($gtt,"account_id")){
			if(ttn($tt,"@id")){
				...
				if($mas->mas_row_cnt){
					...
					//$headers .= "Bcc: colin@anlanda.com\n";
					$headers .= "From: SeedLiving <".SITEEMAIL.">\n";
					$subject = "SeedLiving - Trade Table request has been completed for ".ttn($temptt,"seed_title");
					mail(NULL,$subject,ttn($tt,"message"),$headers);
