...
$(document).ready(function()
{
    ...
    if( (loc = slivPathGetName()) ) {
        if( loc == "login-1" ) {
            login();
        } else if( eval("typeof " + loc + " == 'function'") ) {
            eval(loc+"()");
        } else {
            loc = loc.split("-");
...
function myseedsList() 
{
	...
	$("#seed_tradeopt").change(function(){
		if($(this).val()=="T"){
			var answer = confirm("Putting your item on the trading table means it is available at no cost. You will not be required to enter a price. Are you sure you want to do this?");
			if(answer){
	...
	$('#slSetFeature').click(function() { 
		if($("#featureSeedSelect").val()){
			var url = slivPathMake( "setFeatured-"+$("#featureSeedSelect").val() );
	...
	$('.slRemoveFeature').click(function() { 
		var url = slivPathMake( "removeFeatured-"+$(this).attr("seedid") );
		window.open( url, "_self" );
	});
}
		...
		if($(this).val()){
		  ...
		  if(cartid) {
			  $("#slCartActions")[0].selectedIndex = 0;
			   ...
			   if(cartC>0) $("#cartTotal").html("<a href=\""+slivPathMake("mycart")+"\">"+cartC+"</a>");
			   else {
					window.open( slivPathMake("emptycart"),"_self");   
			   }
		  }
...
function login()
{
	SEEDSessionUser_Login( { urlSuccess: slivPathMake( "accountProfile" ),
							 urlAuth:    slivPathMake( "accountJXAuthenticate" )
							} );
}
function account(){
	...
	$('#registerButton').click(function() { 
		...
		$.ajax({
			type: "POST",
			url: slivPathMake("saveAccount"),
			data: formData,
			success: function(data){
				init();
				if(data) $(".failure").html(data);
				else window.open(slivPathMake("login"),"_self");
			}
		});
	});
}
