...
require_once( SEEDCOMMON."mbr/mbrOrder.php" );
...
switch( $_POST['payment_status'] ) {
    case "Completed":
        if( $kfr->value('eStatus') == MBRORDER_STATUS_NEW ) {
            $kfr->SetValue('eStatus', MBRORDER_STATUS_PAID );
            //$kfr->SetValue('pay_status', MBR_PS_PAID );
        } else {
            $sLog = "Order $rowid got PayPal Completed but eStatus is =".$kfr->value('eStatus');
            $ipn->log( $sLog );
            $ipn->mail( $sLog );
        }
        break;
