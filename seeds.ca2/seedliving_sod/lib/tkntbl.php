...
// This function is only called to make the breadcrumbs record (recording the page inputs).
// Since the code below throws a Notice when $value is an array (e.g. saving the user record, user_image is an array)
// it's better to just throw out the array before the warning appears.
...
function GetTokensRA( $raTT )
{
    $ra = array();
    foreach( $raTT as $tt ) {
        foreach( $tt->tkn as $k => $v ) {
            $ra[$k] = $v;
        }
    }
    return( $ra );
