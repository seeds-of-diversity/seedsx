-- Thanks to hobodave and CurtainDog on StackExchange for these functions.
--
-- replaceword replaces the given anycase word in a string with the givencase word
--    e.g.  replaceword( 'My Phrase', 'phrase' )  returns "My phrase"
--
-- titlecase uses this to replace words with their Titlecase equivalents, except for defined small words, and defined specialcase words
--    e.g.  titlecase( 'HERE\'S A TITLE ABOUT SQL' )  returns "Here's a Title About SQL"
...
DELIMITER | 
CREATE FUNCTION titlecase( str VARCHAR(128) ) 
RETURNS VARCHAR(128)
DETERMINISTIC
BEGIN 
  DECLARE c CHAR(1); 
  ...
  DECLARE punct CHAR(27) DEFAULT ' ()[]{},.-_!@;:?/"#$%^&*<>';  -- This had '' (a single-quote) in the list but that made HERE'S turn into Here'S
